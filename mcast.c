...
void handleDataMessage(message *m, int bytes) {
    ...
    windowSlot ws = (windowSlot) dp->ws;
        ...
        case STATE_SENDING:
            ...
            if (putInBuffer(m)) {
                if (ws.index > currentSession.lastInOrderReceivedIndexes[m->pid - 1]) {
                    u_int32_t currentPointer = getPointerOfIndex(currentSession.lastInOrderReceivedIndexes[m->pid - 1]);
                    u_int32_t lastInOrderIndex = currentSession.dataMatrix[m->pid][currentPointer];
                    while (currentPointer != getPointerOfIndex(m->pid - 1)) {
                        if (currentSession.dataMatrix[m->pid - 1][currentPointer].index < lastInOrderIndex)
                            sendNack(m->pid, currentSession.dataMatrix[m->pid - 1][currentPointer].index);
                    }
                }
            }
...
u_int32_t getPointerOfIndex(u_int32_t pid, u_int32_t index) {
    ...
    return (currentWindowStartPointer + (index - currentWindowSlot[currentWindowStartPointer].index))
...
void deliverLowestData() {
    ...
    deliverToFile(minimumPID, minimumIndex, randomData);
}
...
void prepareFile() {
    ...
    if ((session.f = fopen(fileName, "w")) == NULL) {
        perror("fopen");
        exit(0);
    }
}
